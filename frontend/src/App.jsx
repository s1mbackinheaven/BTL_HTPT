...
function AdminLayout({ auth, showToast }) {
  ...
  const [userForm, setUserForm] = useState({ full_name: '', email: '', phone: '', role: 'user', is_active: true });
  ...
  const closeUserModal = () => {
    ...
    setEditingUser(null);
    setUserForm({ full_name: '', email: '', phone: '', role: 'user', is_active: true });
  };
  ...
  const saveUser = async (e) => {
    ...
    try {
      ...
      showToast(err?.response?.data?.detail || 'Cập nhật user thất bại', 'error');
    }
  };
  ...
  const toggleUserActive = async (user) => {
    try {
      ...
      showToast(user.is_active ? 'Đã deactive tài khoản' : 'Đã active tài khoản', 'success');
      loadAll();
    } catch (err) {
      showToast(err?.response?.data?.detail || 'Cập nhật trạng thái user thất bại', 'error');
    }
  };
...
{userModalOpen && <UserModal form={userForm} setForm={setUserForm} onClose={closeUserModal} onSubmit={saveUser} />}
...
function UserModal({ form, setForm, onClose, onSubmit }) { return <div className="fixed inset-0 z-[96] bg-slate-950/40 backdrop-blur-sm"><div className="mx-auto mt-24 w-[min(92vw,680px)] rounded-[2rem] bg-white p-6 shadow-[0_24px_80px_rgba(15,23,42,0.24)]"><div className="flex items-start justify-between"><div><div className="text-xs font-semibold uppercase tracking-[0.18em] text-brand-700">Chỉnh sửa user</div><div className="mt-1 text-2xl font-semibold text-slate-900">Cập nhật thông tin người dùng</div></div><button onClick={onClose} className="rounded-2xl border border-stone-200 p-2 text-slate-600 hover:bg-stone-50"><X size={18} /></button></div><form onSubmit={onSubmit} className="mt-5 grid gap-4 sm:grid-cols-2"><Field label="Họ tên" value={form.full_name} onChange={(v) => setForm((p) => ({ ...p, full_name: v }))} /><Field label="Email" value={form.email} onChange={(v) => setForm((p) => ({ ...p, email: v }))} /><Field label="Số điện thoại" value={form.phone} onChange={(v) => setForm((p) => ({ ...p, phone: v }))} /><label className="grid gap-2"><span className="text-sm font-medium text-slate-700">Role</span><select value={form.role} onChange={(e) => setForm((p) => ({ ...p, role: e.target.value }))} className="rounded-2xl border border-stone-200 bg-stone-50 px-4 py-3 outline-none focus:border-brand-400 focus:bg-white"><option value="user">user</option><option value="admin">admin</option></select></label><label className="sm:col-span-2 flex items-center gap-3 rounded-2xl border border-stone-200 bg-stone-50 px-4 py-3"><input type="checkbox" checked={!!form.is_active} onChange={(e) => setForm((p) => ({ ...p, is_active: e.target.checked }))} className="h-4 w-4" /><span className="text-sm font-medium text-slate-700">Tài khoản đang active</span></label><div className="sm:col-span-2 flex justify-end gap-3"><button type="button" onClick={onClose} className="rounded-2xl border border-stone-200 px-5 py-3 font-semibold text-slate-700 hover:bg-stone-50">Hủy</button><button className="rounded-2xl bg-brand-600 px-5 py-3 font-semibold text-white hover:bg-brand-700">Lưu thay đổi</button></div></form></div></div>; }
